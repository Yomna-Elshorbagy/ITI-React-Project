 ...
 //each grid(left and right) is spliced into 2 part using chunkImages
  ...
  return (
    <div className="bg-white py-12 px-4 text-center">
      ...
      {/* Image Grid */}
      <div className="flex flex-col md:flex-row justify-center items-center gap-6 md:gap-10">  {/*all the grids sitting in ro in big screen and op top of each other in small screens */}
        {/* Left Side: 2x2 grid */}
        <div className="flex flex-col gap-4">
          {leftChunks.map((row, rowIndex) => (   //row index
            <div key={`left-row-${rowIndex}`} className="flex gap-4">
              {row.map((img, i) => (  //img and the index
                <img
                  key={`left-img-${i}`}
                  src={img}
                  alt={`Left ${rowIndex}-${i}`}
                  className="w-24 h-28 md:w-32 md:h-36 object-cover "  //the center image is double that h and w
                  loading="lazy"
                />
              ))}
            </div>
        ...
        <div>
          <img
            src={centerImage}
            alt="Center"
            className="w-48 h-60 md:w-72 md:h-80 object-cover "
            loading="lazy"
          />
        </div>
